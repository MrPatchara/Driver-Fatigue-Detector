    ...
    try:
        if firebase_connected:
            test_data = {
                ...
                "timestamp": datetime.now().isoformat(),
                "test_number": time.time()
            }
            test_url = f"{FIREBASE_URL}/tests/{DEVICE_ID}.json"
            print(f"[Firebase Test] Sending test data to: {test_url}")
            print(f"[Firebase Test] Payload: {test_data}")
            response = requests.put(test_url, json=test_data, timeout=10)
            print(f"[Firebase Test] Status code: {response.status_code}")
            print(f"[Firebase Test] Response body: {response.text}")
            if response.status_code == 200:
                messagebox.showinfo("Firebase Test", f"✅ Firebase connection successful!\nDevice ID: {DEVICE_ID}")
                return True
            else:
                messagebox.showerror("Firebase Test", f"❌ Firebase connection failed!\nStatus: {response.status_code}")
                return False
        else:
            messagebox.showerror("Firebase Test", "❌ Firebase not connected!")
            return False
    except Exception as e:
        messagebox.showerror("Firebase Test", f"❌ Firebase test failed!\nError: {str(e)}")
        return False

#-- UI Button for Testing Firebase Connection
def add_firebase_test_button(button_container, text_color):
    from tkinter import Button
    try:
        button = Button(button_container, text="TEST FIREBASE", font=("Segoe UI", 10, "bold"),
                        command=test_firebase_connection, bg="#9C27B0", fg=text_color,
                        relief="raised", bd=2, pady=8, cursor="hand2")
        button.pack(pady=3, fill="x")
        print("[Firebase] Test button created")
    except Exception as e:
        print(f"[Firebase] Button error: {e}")
