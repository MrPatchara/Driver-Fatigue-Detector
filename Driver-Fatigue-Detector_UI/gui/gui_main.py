...
def start_gui():
    ...
    # Control buttons
    control_frame = Frame(right_frame, bg=card_bg, relief="raised", bd=2)
    control_frame.pack(fill="x", pady=(0, 5))
    Label(control_frame, text="SYSTEM CONTROLS", font=("Segoe UI", 11, "bold"), fg=accent_color, bg=card_bg).pack(pady=(8, 8))

    button_container = Frame(control_frame, bg=card_bg)
    button_container.pack(pady=(0, 10), padx=15, fill="x")

    start_button = tk.Button(button_container, text="START DETECTION", font=("Segoe UI", 10, "bold"), 
                             command=start_video, bg=success_color, fg=text_color)
    start_button.pack(pady=3, fill="x")

    stop_button = tk.Button(button_container, text="STOP DETECTION", font=("Segoe UI", 10, "bold"), 
                            command=stop_video, bg=danger_color, fg=text_color, state="disabled")
    stop_button.pack(pady=3, fill="x")

    reset_button = tk.Button(button_container, text="RESET COUNTERS", font=("Segoe UI", 10, "bold"), 
                             command=reset_values, bg=warning_color, fg=text_color)
    reset_button.pack(pady=3, fill="x")

    exit_button = tk.Button(button_container, text="EXIT SYSTEM", font=("Segoe UI", 10, "bold"), 
                            command=exit_program, bg=secondary_color, fg=text_color)
    exit_button.pack(pady=3, fill="x")

    add_firebase_test_button(button_container, text_color)
